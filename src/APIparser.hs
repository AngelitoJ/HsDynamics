...
import Text.Parsec.Numbers (parseFloat,parseIntegral)
...
interactWith job project mol = 
  case job of 
     Molcas molcasProject  -> do
                 writeMolcasXYZ (molcasProject ++ ".xyz") mol
                 launchMolcas molcasProject
                 parseMolcas molcasProject ["Grad","Roots"] mol
...
updateCoeffEnergies energies coeff mol = 
  let currentCoeff = getCoeffCI mol
      currentEner  = getEnergy mol
  in  if length currentCoeff < 3 
         then let newCoeffs = coeff : currentCoeff
                  newEnergy = energies : currentEner
              in mol{getCoeffCI=newCoeffs,getEnergy=newEnergy}
         else let newCoeffs = coeff : (tail currentCoeff)
                  newEnergy = energies : (tail currentEner)                                         
              in  mol{getCoeffCI=newCoeffs,getEnergy=newEnergy}  
...
getAlaskaGrad xs = if null xs then ParseInfo [Nothing]
                                   ...
                                   in case ys of
                                           [] -> ParseInfo [Nothing]
                                           [AlaskaMolecularGradients _label t] -> return $ ("Grad",tuples2List t)
--                                            
...
updateMultiStates file mol = do
  ...
  where numat = DL.length $  getAtoms mol
        sh = extent $ getForce mol
...
parseInternals :: GenParser Char st (V.Vector InternalCoord)
parseInternals = do
...
parseSection :: Int ->  GenParser Char st (V.Vector InternalCoord)
parseSection !n = do
  spaces
  nInternals <- parseIntegral
  skipLine
...
parseQ :: Int ->  GenParser Char st InternalCoord 
parseQ !n = do 
           xs <- count n (spaces >> parseIntegral)
           skipLine
...
parseMoleculeXYZ :: GenParser Char st [(Label,[Double])]  
parseMoleculeXYZ = do
    numat <- parseIntegral 
    count 2 skipLine
    geometry <- count (fromInteger numat) parseAtoms
    return $ geometry 
    

parseAtoms :: GenParser Char st (Label,[Double])   
parseAtoms = do 
            spaces 
...
printGnuplot matrix mol = do
  let fun = head . getEnergy
      es = concatMap (printf "%.5f  ") . fun $ mol
      st = printf " %.5f " $ uncurry (!!) $ fun &&& calcElectSt $ mol
...
writeGaussJob theoryLevel project mol =  do
  ...
  let l1 = addNewLines 1 $ "%chk=" ++ project ++ ".chk"
      ...
      weights = if getElecSt mol == Left S0 then "" else addNewLines 5 $ " 0.5       0.5"
