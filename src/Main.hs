...
molcas opts vel =  do
  ...
  molcasDriver getter opts job $ addExternalVec initialMol vel
  
molcasTinker :: Options -> Maybe Vec -> IO ()
molcasTinker opts vel = do
  let files@[input,tinkerKey,molcasFile] =  optInput opts      
  TopData getter temp   <- topData opts    
  ...
  molcasDriver getter opts (MolcasTinker molcasInput molcasQM) $ addExternalVec initialMol vel
...
gaussian opts vel = do
  let [input,fchk,out] = optInput opts 
  TopData getter temp   <- topData opts          
...
processVerletGaussVel opts = do
  let files@[input,fchk,out,velxyz] =  optInput opts
  vs  <- readInitialVel velxyz
